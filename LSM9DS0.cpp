...
LSM9DS0::LSM9DS0(I2C_Device *_adapter, uint8_t gAddr, uint8_t xmAddr):
	I2C_io(_adapter),
	interfaceMode(MODE_I2C),
	gx(0), gy(0), gz(0),
	ax(0), ay(0), az(0),
	mx(0), my(0), mz(0)
{
...
uint16_t LSM9DS0::begin(gyro_scale gScl, accel_scale aScl, mag_scale mScl, 
						gyro_odr gODR, accel_odr aODR, accel_abw aABW, mag_odr mODR)
{	
	...
	int32_t gTest = gReadByte(WHO_AM_I_G);		// Read the gyro WHO_AM_I
	int32_t xmTest = xmReadByte(WHO_AM_I_XM);	// Read the accel/mag WHO_AM_I
	...
	setAccelABW(aABW); //Set the accel anti alias bandwidth
...
void LSM9DS0::gWriteByte(uint8_t subAddress, uint8_t data)
{
	...
	if (interfaceMode == MODE_I2C)
		...
		perror("SPI interface not yet implemented");
		exit(1);
...
void LSM9DS0::xmReadBytes(uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	...
	////////////////////////////////////////
	// SPI Functions NOT YET IMPLEMENTED!!//
	////////////////////////////////////////
